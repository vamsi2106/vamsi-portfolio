      ...
      <article className="py-28">
        <Container className="max-w-3xl">
          ...
          <AnimatedSection className="mt-8">
            {project.context && (
              <span className="mb-3 inline-block rounded bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                {project.context}
              </span>
            )}
          ...
          <AnimatedSection className="mt-12" delay={0.05}>
            <h2 className="text-sm font-semibold uppercase tracking-widest text-navy-600 dark:text-navy-300">
              The problem
            </h2>
            ...
            <ul className="mt-4 space-y-3">
              {project.build.map((point) => (
                <li key={point} className="flex gap-3 text-slate-600 dark:text-slate-300">
                  <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-navy-500" aria-hidden="true" />
          ...
          <div className="mt-16 flex flex-col gap-4 border-t border-slate-200 pt-8 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Next project: <span className="font-medium text-slate-700 dark:text-slate-300">{next.title}</span>
            </p>
